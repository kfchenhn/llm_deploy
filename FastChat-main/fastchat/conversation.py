...
class Conversation:
    ...
    def get_prompt(self) -> str:
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            return ret
        # chenkunfeng
        elif self.sep_style == SeparatorStyle.CHATMLQANY:
            ret = "" if system_prompt == "" else system_prompt + self.sep2 + "\n"
            for role, message in self.messages:
                if message:
                    ret += role + "\n" + message + self.sep + "\n"
                else:
...
register_conv_template(
    Conversation(
        name="qwen-7b-qanything",
        system_template="<|endoftext|><|im_start|>system\n{system_message}",
