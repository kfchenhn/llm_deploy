...
MODEL_PATH="/home/c205/workspace/models/txt2sql_merge_model_0621" 
MODEL_NAME="Txt2SQL-8B"
API_HOST="0.0.0.0"
API_PORT_NUMBER=8005
...
screen -DRRS controller5 -X stuff "python3 -m fastchat.serve.controller --port 51001 \r"
...
screen -DRRS worker-d5 -X stuff  "CUDA_VISIBLE_DEVICES=0,1 python3 -m fastchat.serve.model_worker --model-path $MODEL_PATH --conv-template txt2sql --num-gpus 2 --port 51002 --worker-address http://localhost:51002 --controller-address  http://localhost:51001 \r"
...
screen -DRRS api5 -X stuff  "python3 -m fastchat.serve.openai_api_server --host $API_HOST --port $API_PORT_NUMBER --controller-address  http://localhost:51001\r"
