...
MODEL_PATH="/home/c205/workspace/models/Qwen-7B-QAnything/" 
API_HOST="0.0.0.0"
...
screen -DRRS controller0 -X stuff  "python3 -m fastchat.serve.controller --port 21001\r"
...
screen -DRRS worker-d0 -X stuff  "CUDA_VISIBLE_DEVICES=0 python3 -m fastchat.serve.model_worker --model-path $MODEL_PATH --port 21002 --worker-address http://localhost:21002 --controller-address  http://localhost:21001  --conv-template qwen-7b-qanything \r"

screen -DRRS api0 -X stuff  "python3 -m fastchat.serve.openai_api_server --host $API_HOST --port $API_PORT_NUMBER --controller-address  http://localhost:21001 \r"
