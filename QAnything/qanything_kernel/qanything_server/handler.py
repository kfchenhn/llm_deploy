...
__all__ = ["new_knowledge_base", "upload_files", "list_kbs", "list_docs", "delete_knowledge_base", "delete_docs",
           ...
           "get_qa_info", "list_available_models"]
...
async def upload_text(req: request):
    ...
    if not is_valid:
        return sanic_json({"code": 2005, "msg": get_invalid_user_id_msg(user_id=user_id)})
    debug_logger.info("upload_text %s", user_id)
    kb_id = safe_get(req, 'kb_id')
    text = safe_get(req,"text")
    #将文本的md5作为file_name
    ...
    exist_files = []
    if mode == 'soft':
        exist_files = local_doc_qa.mysql_client.check_file_exist_by_name(user_id, kb_id, [file_name])
    if exist_files:
        file_id, file_name, file_size, status = exist_files[0]
        msg = f'warning，当前的mode是soft，无法上传同名文件，如果想强制上传同名文件，请设置mode：strong'
        data = [{"file_id": file_id, "text": text, "status": status, "bytes": file_size, "timestamp": timestamp}]
    # 不存在同名文件
    else:
        file_id, msg = local_doc_qa.mysql_client.add_file(user_id, kb_id, file_name, timestamp)
        local_file = LocalFile(user_id, kb_id, text, file_id, file_name, local_doc_qa.embeddings, is_text=True)
        local_doc_qa.mysql_client.update_file_path(file_id, local_file.file_path)

        splited_texts=text_splitter.split_text(text=text)
        data = [{"file_id": file_id,  "splited_texts": splited_texts, "status": "gray", "bytes": 0, "timestamp": timestamp}]
        asyncio.create_task(local_doc_qa.insert_files_to_faiss(user_id, kb_id, [local_file]))
        msg = "success，后台正在飞速上传文件，请耐心等待"
    return sanic_json({"code": 200, "msg": msg, "data": data})
...
#   增加多模型支持
    model_name = safe_get(req,"model")
    if model_name == None :
        model = local_doc_qa.model
    else :
        model = model_name
    # chenkunfeng 区分纯大模型对话/RAG对话
    chat_mode = safe_get(req,"chat_mode")
    ...
    debug_logger.info("chat_mode: %s", chat_mode)
    ...
    if chat_mode == "llm" or (bot_id and local_doc_qa.mysql_client.check_kb_exist(user_id, kb_ids)) or len(kb_ids) == 0:
        ...
        if streaming:
            ...
            debug_logger.info("test1:%s",history)
            async def generate_answer(response):
                debug_logger.info("start generate...")
                async for resp, next_history in local_doc_qa.get_llm_answer(custom_prompt=custom_prompt,
                                                                                        query=question, 
            ...
            async for resp, history in local_doc_qa.get_llm_answer(custom_prompt=custom_prompt,
                                                                               query=question, 
                                                                               chat_history=history, 
                                                                               streaming=False, 
                                                                               need_web_search=need_web_search,
            ...
            qa_logger.info("chat_data: %s", chat_data)
            debug_logger.info("response: %s", chat_data['result'])
            return sanic_json({"code": 200, "msg": "success chat", "question": question, "response": resp["result"],"tokens":resp["tokens"],
                               "history": history, "source_documents": source_documents,"model":model_name})
    ...
    # chenkunfeng 知识库模式
        ...
        if len(valid_files) == 0:
            ...
            if streaming:
                debug_logger.info("start generate answer")
                debug_logger.info("test2:%s",history)
                async def generate_answer(response):
                    debug_logger.info("start generate...")
...
# # chenkunfeng 获取可用的大模型
async def list_available_models(req: request):
    local_doc_qa: LocalDocQA = req.app.ctx.local_doc_qa
    user_id = safe_get(req, 'user_id')
    if user_id is None:
        return sanic_json({"code": 2002, "msg": f'输入非法！request.json：{req.json}，请检查！'})
    ...
    if not is_valid:
        return sanic_json({"code": 2005, "msg": get_invalid_user_id_msg(user_id=user_id)})
    model_list = local_doc_qa.available_model
    debug_logger.info("all available models: {}".format(model_list))
    return sanic_json({"code": 200, "data": model_list})
