...
from .handler import upload_text
...
from qanything_kernel.configs.model_config import DT_7B_MODEL_PATH, \
    DT_7B_DOWNLOAD_PARAMS, DT_3B_MODEL_PATH, DT_3B_DOWNLOAD_PARAMS, CHATGLM_MODEL_PATH ,MODEL_NAMES
...
parser.add_argument('--openai_api_base', dest='openai_api_base', default='https://api.chatanywhere.tech/v1', type=str,
...
if not args.use_openai_api:
    ...
    elif model_size == "6B_GLM":
        args.model = CHATGLM_MODEL_PATH
        model_download_params = DT_7B_DOWNLOAD_PARAMS
    else:
...
app.static('/qanything/', '/home/c205/workspace/QAnything/qanything_kernel/qanything_server/dist/qanything/', name='qanything', index="index.html")
...
async def switch_llm(req):
    model_name = safe_get(req,"model")
    if model_name == "all":
        return sanic_response.json({"supported models":MODEL_NAMES})
    if model_name not in MODEL_NAMES:
        raise(f"not support llm{model_name}")
    app.ctx.local_doc_qa.switch_llm(model_name)
    return sanic_response.json({"status": "model switched"})
...
app.add_route(upload_text, "/api/local_doc_qa/upload_text", methods=['POST'])  # tags=["上传文本"] 
...
app.add_route(list_available_models, "/api/local_doc_qa/list_available_models", methods=['POST'])  # tags=["可用大模型列表"] 
