...
from qanything_kernel.configs.model_config import MODEL_NAMES
import easyocr
...
class LocalDocQA:
    def __init__(self):
        ...
        # chenkunfeng 可用的大模型
        self.available_model: List[str] = MODEL_NAMES
    ...
    def switch_llm(self,model_name):
        if model_name not in MODEL_NAMES:
            debug_logger.info(f"not support llm{model_name}")
        else:
            self.llm.switch_llm(model_name)
...
#         debug_logger.info(f"语言模型{llm}")
#         return llm

    
# #加载多个llm
#     def init_cfg(self, args=None):
#         all_args = [copy.deepcopy(args) for _ in range(len(args.model))]
#         # all_args = [args]*len(args.model)#浅拷贝，指向同一个对象
#         for i in range(len(args.model)):
#             # debug_logger.info(args)
#             # debug_logger.info(i)
#             all_args[i].model_size = args.model_size[i]
#             all_args[i].device_id = args.device_id[i]
#             all_args[i].model = args.model[i]
#             all_args[i].gpu_memory_utilization = args.gpu_memory_utilization[i]
#             self.rerank_top_k = int(all_args[i].model_size[0])
#             self.use_cpu = all_args[i].use_cpu
#             if all_args[i].use_openai_api:
#                 self.model = all_args[i].openai_api_model_name
#             else:
#                 self.model = all_args[i].model[i].split('/')[-1]
#             self.models.append(self.model)
#             # process = multiprocessing.Process(target=self.load_model,args=(all_args[i],))
#             self.load_model(all_args[i])
#             if platform.system()=="Linux":
#                 from qanything_kernel.connector.rerank.rerank_onnx_backend import RerankOnnxBackend
#                 from qanything_kernel.connector.embedding.embedding_onnx_backend import EmbeddingOnnxBackend
#                 self.local_rerank_backend: RerankOnnxBackend = RerankOnnxBackend(self.use_cpu)
...
#             self.faiss_client = FaissClient(self.mysql_client, self.embeddings)
    ...
    # if platform.system() == 'Linux':
            #     if all_args[i].use_openai_api:
            #         self.llm: OpenAILLM = OpenAILLM(all_args[i])
            #     else:
            #         from qanything_kernel.connector.llm.llm_for_fastchat import OpenAICustomLLM
            #         self.llm: OpenAICustomLLM = OpenAICustomLLM(all_args[i])
            #     self.llms.append(self.llm)
            #     debug_logger.info(f"语言模型{self.llm}")
                # from qanything_kernel.connector.rerank.rerank_onnx_backend import RerankOnnxBackend
                # from qanything_kernel.connector.embedding.embedding_onnx_backend import EmbeddingOnnxBackend
                # self.local_rerank_backend: RerankOnnxBackend = RerankOnnxBackend(self.use_cpu)
                # self.embeddings: EmbeddingOnnxBackend = EmbeddingOnnxBackend(self.use_cpu)
            ...
            #     else:
            #         from qanything_kernel.connector.llm.llm_for_llamacpp import LlamaCPPCustomLLM
            #         self.llm: LlamaCPPCustomLLM = LlamaCPPCustomLLM(all_args[i])
            #     self.llms.append(self.llm)
            #     debug_logger.info(f"语言模型{self.llm}")
                # from qanything_kernel.connector.rerank.rerank_torch_backend import RerankTorchBackend
                # from qanything_kernel.connector.embedding.embedding_torch_backend import EmbeddingTorchBackend
                # self.local_rerank_backend: RerankTorchBackend = RerankTorchBackend(self.use_cpu)
                # self.embeddings: EmbeddingTorchBackend = EmbeddingTorchBackend(self.use_cpu)
            # self.llm = self.load_model(all_args[i])
            # self.llms.append(self.llm)
            # debug_logger.info(f"语言模型{self.llm}")
    #zhuoran 重构初始化函数，单独实现模型加载
    # def init_llm(self,args=None):
    #     debug_logger.info(f"switch llm to {args.model.split('/')[-1]}")
    #     self.rerank_top_k = int(args.model_size[0])
    #     if args.use_openai_api:
    #         self.model = args.openai_api_model_name
    #     else:
    #         self.model = args.model.split('/')[-1]#args.model是他的路径，这里取最后一个名字
    #     if self.model not in self.models:
    #         # 打印错误信息并退出程序
    #         print(args.model)
    #         print(f"Error: model '{self.model}' not exist in {self.models}")
    #         sys.exit(1)  # 退出程序，非0值表示出现错误
    #     else:
    #         # 如果模型存在，则记录加载信息
    #         debug_logger.info(f"loading {self.model} checkpoints")
 ...
 #           debug_logger.info(f"Local doc qa response:{response}")
            yield response, history
        t2 = time.time()
        debug_logger.info(f"LLM time: {t2 - t1}")
        debug_logger.info(f"tokens:{tokens}")



    # chenkunfneg 利用大模型本身的能力回复
    async def get_llm_answer(self, custom_prompt, query,  chat_history=None, 
                                         streaming: bool = STREAMING,
                                         need_web_search: bool = False,
                                         model_name:str = None):
        if chat_history is None:
        ...
        prompt = self.generate_prompt(query=query,
                                      ...
                                      prompt_template=prompt_template)
        t1 = time.time()
        debug_logger.info("test4_history:%s",chat_history)
        async for answer_result in self.llm.generatorAnswer(prompt=prompt,
                                                      history=chat_history,
                                                      streaming=streaming,
                                                      model_name= model_name):
            ...
            response = {"query": query,
                        ...
                        "retrieval_documents": "",
                        "source_documents": ""}
 #           debug_logger.info(f"llm_self response:{response}")
            yield response, history
        t2 = time.time()
        debug_logger.info(f"LLM time: {t2 - t1}")
        debug_logger.info(f"tokens:{tokens}")
