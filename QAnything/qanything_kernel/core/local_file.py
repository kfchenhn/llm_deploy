...
class LocalFile:
    def __init__(self, user_id, kb_id, file: Union[File, str, dict], file_id, file_name, embedding, is_url=False,is_text=False, in_milvus=False):
    ...
    def split_file_to_docs(self, ocr_engine: Callable, sentence_size=SENTENCE_SIZE,
        ...
        if self.url:
            ...
            splited_texts=text_splitter.split_text(text=self.file_content)
            docs = [Document(page_content=splited_text, metadata={"splited_text": splited_text}) for splited_text in splited_texts]
