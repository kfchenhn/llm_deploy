...
from qanything_kernel.configs.model_config import DT_CONV_7B_TEMPLATE, DT_CONV_3B_TEMPLATE,CHATGLM_MODEL_TEMPLATE
...
class OpenAICustomLLM(BaseAnswer, ABC):
    ...
    def __init__(self, args):
        ...
        elif args.model_size =='6B_GLM':
            self.conv_template = CHATGLM_MODEL_TEMPLATE
    ...
    async def _call(self, prompt: str, history: List[List[str]], streaming: bool = False) -> str:
        ...
        debug_logger.info("fastchat_prompt:%s",prompt)
    ...
    async def generatorAnswer(self, prompt: str,
        ...
        debug_logger.info(f"all_history:{history}")
        ...
        async for response_text in response:
            ...
            answer_result.tokens = self.num_tokens_from_messages([prompt])
            yield answer_result
