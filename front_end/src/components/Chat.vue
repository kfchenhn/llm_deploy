...
const getOptionList = async () => {
  ...
  modelName.value = data.data.data[0];
};
const setModelName = (e, value) => {
  modelName.value = value;
};
const setChat_mode = (e, value) => {
  chat_mode.value = value;
};
getOptionList();
pubsub.subscribe('setModelName', setModelName);
pubsub.subscribe('setChat_mode', setChat_mode);
//当前是否开启链网检索
...
const send = () => {
  ...
  let body = {
    ...
    question: q,
    model: modelName.value, //具体模型
    streaming: true,
    networking: network.value,
    product_source: 'saas',
    chat_mode: chat_mode.value, //hander类别
  };
  if (chat_mode.value !== 'llm') {
    body.kb_ids = selectList.value;
  }
